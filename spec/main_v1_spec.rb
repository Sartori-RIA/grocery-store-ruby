...
describe "MainV1" do
  ...
  describe "#format_order_items" do
    ...
    it { expect(subject[0]).to eq({ quantity: 1, price: 20, name: "1 cake", is_imported: false, category: :food }) }
    it { expect(subject[1]).to eq({ quantity: 2, price: 15.90, name: "2 chocolate", is_imported: false, category: :food }) }
    it { expect(subject[2]).to eq({ quantity: 3, price: 1000, name: "3 imported PS5", is_imported: true, category: :other }) }
  end
