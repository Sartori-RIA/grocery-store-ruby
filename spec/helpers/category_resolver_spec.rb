...
RSpec.describe CategoryResolver do
  describe '#constants' do
    ...
    it {
      expect(described_class::CATEGORIES[:medical]).to eq(%w[pill pills headache medicine syrup])
    }
    it {
      expect(described_class::CATEGORIES[:food]).to eq(['chocolate', 'chocolates', 'bread', 'cake',
                                                        'apple', 'hot dog', 'hot-dog', 'pizza', 'meat', 'rice', 'beans', 'burger'])
    }
  end
